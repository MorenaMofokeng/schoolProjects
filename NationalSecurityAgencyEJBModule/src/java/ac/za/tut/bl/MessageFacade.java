...
public class MessageFacade extends AbstractFacade<Message> implements MessageFacadeLocal {
    ...
    @Override
    @RolesAllowed("agent_manager")
    public Message findLongest() {
        Query query = em.createQuery("SELECT m FROM Message m WHERE " +
                "m.msgLength = (SELECT MAX(msg.msgLength) FROM Message msg)");
